...
public class FullingBaseAnyValues {
    ...
    private final ContractorService contractorService = ContractorServiceImpl.getInstance();
    ...
    private List<ProposalDto> getAllConsiderationsProposals() {
        ...
        List<DeveloperDto> allActiveDevelopers = new ArrayList<>(adminService.getAllDevelopers(FilterPageDto.builder()
                ...
                allProjectList.forEach(project -> {
                    try {
                        activeChapterList.addAll(
                            developerService.getChaptersByProject(FilterPageDto.builder()
                                                                      .id(project.getId())
                                                                      .build()).getList());
        ...
        activeChapterList.forEach(chapter -> {
            try {
                proposalList.addAll(
                    developerService.getProposalsByChapter(FilterPageDto.builder()
                                                               .id(chapter.getId())
        ...
        calculationList.forEach(calculation -> {
            try {
                if (calculationIndex.get() % INT2 == INT0) {
                    ...
                    developerService.payMoney(CreateRequestDto.builder()
            ...
            .forEach(chapterId -> {
                ...
                for (int i = 0; i < thisCalculationCount; i++) {
                    ...
                    try {
                        ...
                        contractorService.createCalculation(CreateRequestDto.builder()
                                                                .id(chapterId)
                                                                .int1(BOUND4)
                                                                .int2(mm)
                                                                .int3(workPricePlan)
                ...
                try {
                    ...
                    list.forEach(calculation -> {
                        ...
                        try {
                            contractorService.updateWorkPriceFact(ChangeRequestDto.builder()
                                                                      .id(calculation.getId())
                                                                      .count(workPriceFact)
                                                                      .build());
                        } catch (Exception e) {
    ...
    private List<Long> startBusiness(List<ProposalDto> approvedList) {
        ...
        return approvedList.stream()
                   .peek(proposal -> {
                       try {
                           contractorService.setProposalStatus(ChangeRequestDto.builder()
                                                                   .id(proposal.getId())
        ...
        for (int i = 0; i < stop; i++) {
            try {
                ...
                DtoWithPageForUi<ProposalDto> proposalDtoDtoWithPageForUi =
                    developerService.changeStatusOfProposal(ChangeRequestDto.builder()
                                                                .id(proposal.getId())
    ...
    private void changeProjectStatus(List<Project> allProjectList) {
        ...
        allProjectList.forEach(project -> {
            if (projectIndex.get() % INT2 != INT0) {
                try {
                    developerService.changeProjectStatus(ChangeRequestDto.builder()
                                                             .id(project.getId())
    ...
    private void creationOfBaseData() {
        ...
        adminService.createAdmin(CreateRequestDto.builder()
                                     .email(getUniqEmail())
                                     .password(getPassword())
                                     .build());
        for (int i = 0; i < DEVELOPER_COUNT; i++) {
            long devId = developerService.createDeveloper(CreateRequestDto.builder()
                                                              .email(getUniqEmail())
                                                              .password(getPassword())
                                                              .name(getUserName())
            ...
            for (int j = 0; j < thisProjectCount; j++) {
                developerService.createProject(CreateRequestDto.builder()
                                                   .id(devId)
                                                   .name(getProjectName())
                ...
                for (int k = 0; k < thisChapterCount; k++) {
                    developerService.createChapter(CreateRequestDto.builder()
                                                       .id((long) projectCount)
                                                       .name(CHAPTER_NAMES[k])
                                                       .int1(RANDOM.nextInt(BOUND2))
                                                       .build());
                    if (thisChapterCount % INT5 == 0) {
                        developerService.cancelChapter((long) chapterCount);
                    }
                    chapterCount++;
                }
                projectCount++;
        ...
        for (int i = 0; i < CONTRACTOR_COUNT; i++) {
            ...
            long conId = contractorService.createContractor(CreateRequestDto.builder()
                                                                .email(getUniqEmail())
                                                                .password(getPassword())
                                                                .name(getUserName())
                                                                .city(CITY_NAMES[i % CITY_NAMES.length])
            ...
            for (int j = 0; j < thisProposalCount; j++) {
                DtoWithPageForUi<ProposalDto> proposal = contractorService.createProposal(CreateRequestDto.builder()
    ...
    private String getStreet() {
        ...
        return WEST_AVENUE + userCount % INT7;
    }
